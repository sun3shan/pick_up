...
class Environment:
    ...
    def reset(self, obs_cnt = 20, pkg_cnt = 24):
        ...
        if False: #use random map
            while True:
                ...
                self.map[tuple(self.rand.randint(0,self.map_size,obs_cnt)),tuple(self.rand.randint(0,self.map_size,obs_cnt))] = -1
                self.map[0,0] = 0
    ...
    def move(self, movdir):
        ...
        self.step += 1
        self.GameOver = True if self.step == self.max_step else False
    ...
    def __add_package(self, pkg_cnt = 1):
        ...
        cnt = pkg_cnt
        while cnt>0:
            i = self.rand.randint(0, r-1)
            j = self.rand.randint(0, c-1)
            
            if self.map[i,j] == 0 and i!=self.posx and j!=self.posy:
                self.map[i,j] = self.rand.randint(18,48)
                cnt-=1
